 ...
 *
 * Binary layout (big-endian, padded to 20 bytes):
 ...
 *   offset 17  padding     3 bytes  reserved (zero)
 */
public final class HeapTupleHeader {
    ...
    public HeapTupleHeader(RecordId ctid) {
    ...
    private HeapTupleHeader(int xmin, int xmax, RecordId ctid, short infomask, byte hoff) {
        this.xmin     = xmin;
        this.xmax     = xmax;
        this.ctid     = ctid;
    ...
    public static HeapTupleHeader readFrom(ByteBuffer buf, int offset) {
        ...
        return new HeapTupleHeader(xmin, xmax, new RecordId(ctidPage, ctidSlot), infomask, hoff);
    }
